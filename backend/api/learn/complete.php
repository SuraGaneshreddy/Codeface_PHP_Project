...
$lesson = db_one('SELECT id, track, position FROM learn_lessons WHERE id = ?', [$lessonId]);
if (!$lesson) json_error('Lesson not found.', 404);

/* hard level locking: completion of a lesson inside a locked level is refused */
$lvSlug = cf_learn_level_of_position((int)$lesson['position']);
if (!cf_learn_level_unlocked($lesson['track'], $lvSlug, (int)$me['id'])) {
    json_error('This level is locked — finish the previous level first.', 403, ['locked' => true, 'level' => $lvSlug]);
}
