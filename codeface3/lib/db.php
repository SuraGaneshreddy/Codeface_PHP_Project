 ...
 *  - sqlite: creates data/codeface.sqlite, runs schema, seeds demo data on first use
...
function db(): PDO {
    ...
    if ($driver === 'mysql') {
        ...
        if (!$hasUsers) {
            db_run_sql_file($pdo, __DIR__ . '/../sql/schema.mysql.sql');
            seed_database($pdo);
        } else {
        ...
        if ($fresh) {
            db_run_sql_file($pdo, __DIR__ . '/../sql/schema.sqlite.sql');
            seed_database($pdo);
        }
    }
