...
/* AI-made problems are private to the user they were generated for. */
if ($problem && $problem['ai_user_id'] !== null && (int)$problem['ai_user_id'] !== (int)$me['id']) {
    $problem = null; // behave exactly like "not found" — don't leak existence
}
if (!$problem) {
...
$ownSql = '(ai_user_id IS NULL OR ai_user_id = ?)';
$ownParams = [(int)$me['id']];
$next = db_one("SELECT slug, title FROM problems WHERE id > ? AND $ownSql ORDER BY id ASC LIMIT 1", array_merge([$problem['id']], $ownParams));
$prev = db_one("SELECT slug, title FROM problems WHERE id < ? AND $ownSql ORDER BY id DESC LIMIT 1", array_merge([$problem['id']], $ownParams));
...
$moreInCat = $category !== ''
    ? db_all("SELECT slug, title FROM problems WHERE category = ? AND slug != ? AND $ownSql ORDER BY id ASC LIMIT 3", array_merge([$category, $slug], $ownParams))
    : [];
  ...
  <div class="split">
    <section class="pane pane-left">
      <div class="pane-scroll problem-desc">
        ...
        <?php if ($category === 'ai'): ?>
        <div class="ai-hero"><span>🤖</span> This problem was written by the Codeface AI just for you — finish the whole set and it will generate the next 10.</div>
        <?php endif; ?>
    ...
    <section class="pane pane-right">
      <div class="editor-toolbar">
        ...
        <button class="btn btn-ghost btn-sm" id="btnReview" type="button" title="Rule-based senior-engineer review of your code">👨‍💻 Review</button>
...
<?php
$page_scripts = ['assets/js/editor.js', 'assets/js/runner.js', 'assets/js/metrics.js', 'assets/js/reviewer.js', 'assets/js/problem.js'];
require __DIR__ . '/../backend/partials/footer.php';
?>
