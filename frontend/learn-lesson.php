...
/* Hard level locking: a lesson inside a locked level is itself locked (403). */
$levelSlug = cf_learn_level_of_position($n);
if (!cf_learn_level_unlocked($lang, $levelSlug, $me ? (int)$me['id'] : null)) {
    http_response_code(403);
    $lv = cf_learn_levels()[$levelSlug];
    $level = $levelSlug;
...
<div class="container lesson-wrap">
  <nav class="crumbs">
    <?php
    $levelSlug = cf_learn_level_of_position($n);
    $levelMeta = cf_learn_levels()[$levelSlug];
    ?>
    <a href="learn-track.php?l=<?= urlencode($lang) ?>"><?= e($meta['name']) ?></a>
    <span class="crumb-sep">/</span>
    <a href="learn-level.php?l=<?= urlencode($lang) ?>&amp;level=<?= e($levelSlug) ?>"><?= e($levelMeta['icon']) ?> <?= e($levelMeta['name']) ?></a>
