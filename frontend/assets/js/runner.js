...
window.CFRunner = (function () {
  ...
  function spawn(workerFile, msg, opts, resolve) {
    ...
    worker.onmessage = function (ev) {
      ...
      if (m.type === 'loading') {
        ...
        finish({ project: true, results: m.results, logs: m.logs, projectOk: m.ok, error: m.error });
      }
    };
  ...
  /** opts: {files:{name:content}, order:[names], checks:[{expr}], timeout}
   *  → {ok, results:[{i,pass,error?}], logs, error?}  (Labs & Refactor Gym) */
  function runProject(opts) {
    return new Promise(function (resolve) {
      var o = { timeout: opts.timeout || 8000, tests: [] };
      spawn('assets/js/runner-worker.js', {
        mode: 'project', files: opts.files, order: opts.order, checks: opts.checks,
      }, o, function (out) {
        resolve({
          ok: !out.timedOut && out.projectOk !== false,
          results: out.results || [],
          logs: out.logs || '',
          error: out.bootError || out.error || (out.timedOut ? 'Time limit exceeded.' : null),
        });
      });
    });
  }
