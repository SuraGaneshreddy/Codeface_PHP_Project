...
document.addEventListener('DOMContentLoaded', function () {
  ...
  CFEditor.create(host, {
    ...
    function runAll(submit) {
      ...
      CFRunner.run({
        ...
        if (submit && !out.bootError && !out.timedOut) {
          CF.api('../backend/api/submissions.php', {
            body: {
    ...
    /* Senior-engineer review (rule-based, local) */
    var btnReview = document.getElementById('btnReview');
    var reviewPanel = document.getElementById('reviewPanel');
    if (btnReview && reviewPanel && window.CFReviewer) {
      btnReview.addEventListener('click', function () {
        var res = CFReviewer.analyze(editor.getValue(), currentLang);
        ...
        reviewPanel.innerHTML =
          '<div class="card-head"><h3>Senior review <span class="muted">(rule-based — readability, security, design)</span></h3>' +
